...
32 位 ARM（armv6 / armv7，例如装了 32 位系统的树莓派）没有现成镜像——Node 自 24 起不再发布这两个架构。这类设备可以把 [Dockerfile](https://github.com/nemobb/singbox-config/blob/main/Dockerfile) 的基础镜像改成 `node:22-alpine` 后自行构建。
